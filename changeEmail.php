    ...
    if (!empty($currentEmail) && !empty($newEmail)) {
        ...
        if (mysqli_num_rows($sql) > 0) {
            ...
            if ($currentEmail === $user_email) {
                ...
                if ($sql2) {
                    echo '<p class="success-message">Email updated successfully!</p>';
                } else {
                    echo '<p class="error-message">Something went wrong. Please try again!</p>';
                }
            } else {
                echo '<p class="error-message">Current Email is incorrect!</p>';
            }
        }
    } else {
        echo '<p class="error-message">All input fields are required!</p>';
    }
}
?>
